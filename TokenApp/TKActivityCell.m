...
@interface TKActivityCell ()

/*! Private view components */
@property (nonatomic, strong) TKProfileImageView *activityImageView;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        horizontalTextSpace = [TKActivityCell horizontalTextSpaceForInsetWidth:0];
        ...
        self.activityImageView = [[TKProfileImageView alloc] init];
...
- (void)setActivity:(PFObject *)activity {
    ...
    _activity = activity;
    if ([[activity objectForKey:kPTKActivityTypeKey] isEqualToString:kPTKActivityTypeFollow] || [[activity objectForKey:kPTKActivityTypeKey] isEqualToString:kPTKActivityTypeJoined]) {
        [self setActivityImageFile:nil];
    } else {
        [self setActivityImageFile:(PFFile*)[[activity objectForKey:kPTKActivityPhotoKey] objectForKey:kPTKPhotoThumbnailKey]];
    }

    NSString *activityString = [TKActivityFeedViewController stringForActivityType:(NSString*)[activity objectForKey:kPTKActivityTypeKey]];
    self.user = [activity objectForKey:kPTKActivityFromUserKey];

    // Set name button properties and avatar image
    if ([TKUtility userHasProfilePictures:self.user]) {
        [self.avatarImageView setFile:[self.user objectForKey:kPTKUserProfilePicSmallKey]];
    } else {
    ...
    if (self.user && [self.user objectForKey:kTKUserDisplayNameKey] && [[self.user objectForKey:kTKUserDisplayNameKey] length] > 0) {
        nameString = [self.user objectForKey:kTKUserDisplayNameKey];
    }
    ...
    if (self.user) {
        CGSize nameSize = [self.nameButton.titleLabel.text boundingRectWithSize:CGSizeMake(nameMaxWidth, CGFLOAT_MAX)
                                                                     ...
                                                                     attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                                                        context:nil].size;
        NSString *paddedString = [TKBaseCellText padString:activityString withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
...
- (void)setCellInsetWidth:(CGFloat)insetWidth {
    ...
    horizontalTextSpace = [TKActivityCell horizontalTextSpaceForInsetWidth:insetWidth];
}
...
- (id<TKActivityCellDelegate>)delegate {
    return (id<TKActivityCellDelegate>)_delegate;
}

- (void)setDelegate:(id<TKActivityCellDelegate>)delegate {
...
+ (CGFloat)heightForCellWithName:(NSString *)name contentString:(NSString *)content cellInsetWidth:(CGFloat)cellInset {
    CGSize nameSize = [name boundingRectWithSize:CGSizeMake(200.0f, CGFLOAT_MAX)
                                      ...
                                      attributes:@{NSFontAttributeName:[UIFont boldSystemFontOfSize:13.0f]}
                                         context:nil].size;
    NSString *paddedString = [TKBaseCellText padString:content withFont:[UIFont systemFontOfSize:13.0f] toWidth:nameSize.width];
    CGFloat horizontalTextSpace = [TKActivityCell horizontalTextSpaceForInsetWidth:cellInset];
